  ...
  function updateInsightFortaleza(allDims, fac, car, cic) {
    ...
    if (hayFiltro) {
      ...
      if (fortalezas.length > 0) {
        ...
        narrativa += `. En total, <strong>${fortalezas.length}</strong> de ${allDims.length} dimensiones superan el ${META_CSAT} % de satisfacción.`;
      } else {
        narrativa += `Ninguna dimensión alcanza el umbral de <strong>${META_CSAT} %</strong> (Fortaleza). `;
      ...
      if (fortalezas.length > 0) {
        ...
        narrativa += `En total, <strong>${fortalezas.length}</strong> de ${allDims.length} dimensiones se encuentran en rango de Fortaleza (≥${META_CSAT} %).`;
      } else {
        narrativa += `Actualmente ninguna dimensión alcanza el umbral de <strong>${META_CSAT} %</strong> (Fortaleza).`;
      }
      if (atencion.length > 0) {
  ...
  function updateInsightAtencion(data, fac, car, cic) {
    ...
    if (hayFiltro) {
      ...
      if (sorted.length >= 2) {
        ...
        narrativa += `<strong>${fmtDim(lowest.dimension)}</strong> (${fmtPct(lowest.pctNoConozco)} · No conozco + ${fmtPct(lowest.pctNoUtilizo)} · No utilizo) y `;
        narrativa += `<strong>${fmtDim(secondLowest.dimension)}</strong> (${fmtPct(secondLowest.pctNoConozco)} · No conozco + ${fmtPct(secondLowest.pctNoUtilizo)} · No utilizo) `;
        ...
        const [lowest] = sorted;
        narrativa += `<strong>${fmtDim(lowest.dimension)}</strong> (${fmtPct(lowest.pctNoConozco)} · No conozco + ${fmtPct(lowest.pctNoUtilizo)} · No utilizo) es la que presenta <strong>menor visibilidad</strong>.`;
      }
    }
